...
public:
    ...
    int search_image(const std::unique_ptr<tflite::Interpreter>& interpreter, const std::string& img_name, int left_x, int left_y, int box_sz, int model_sz){
...
int main() {
    ...
    // Load images
    cv::Mat left = cv::imread("./left_front5.png");
    cv::Mat right = cv::imread("./right_front5.png");

    // // create model for feature extraction, this piece of code cannot make into functions.
    std::unique_ptr<tflite::FlatBufferModel> model =
        tflite::FlatBufferModel::BuildFromFile("./mobilenetv2_quant_int8.tflite");
    ...
    int optx = cmp.search_image(interpreter, "front5.jpg", 844,387,224, 224);
