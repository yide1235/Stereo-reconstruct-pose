    ...
    def task(self, frameL, frameR, waitKey=5, show_depth=False, use_augmentation=None, left_file_path=None, num_aug=None, output_folder=None, pick_biggest_box=True,run_onetime=None,effective_range=None,max_min=None):
        ...
        # #code for single point
            ...
            if len(bboxes_l)>0:
                ...
                shoulder_point_y=[]
                for rectified_left_points_result in rectified_left_points_results:
                    shoulder_point_x.append(rectified_left_points_result[6][0][0])
                    shoulder_point_y.append(rectified_left_points_result[6][0][1])
                    tempx=median(shoulder_point_x)
                    tempy=median(shoulder_point_y)
                ...
                if points3d[2]<effective_range and conf_sums_local[0]>max_min:
                    ...
                    # print('parts is: ', temp_final )
                    # print('-------')


                    #since only use one bbox for now:
...
def merge(merge_parts):
    ...
    for i in merge_parts:
        ...
        if n>5:
            ...
            range1=temp[3]-temp[1]
            mean1=temp[2]
        elif n==4:
            range1=temp[2]-temp[1]
...
def add_frame(local_database, frame):
    ...
    if len(local_database)==0:
        ...
        for i in range(len(local_database)):
            ...
            if (intersect(means_a, ranges_a, means_b, ranges_b, 0.5, map_probs)[0]>1.000001e-10):

                # for j in local_database:
                #     print('mean is: ', j.mean_local)
                #     print('range is: ', j.range_local)
                #     print('---')
                ...
                frame=merge(merge_parts)

                local_database[i]=frame
                print('successfully merged with same one')

                # for k in local_database:
                #     print('mean is: ', k.mean_local)
                #     print('range is: ', k.range_local)
                #     print('---')
...
def merge_recent_five(local_database, recent_five_frames):

    merge_parts=recent_five_frames[0].parts_local
    for i in range(len(recent_five_frames)-2):
        merge_parts=merge_lists(merge_parts, recent_five_frames[i+1].parts_local)

    
    local_database=add_frame(local_database,merge(merge_parts))

    return local_database

    # return means, ranges, result_part
...
if __name__ == '__main__':
    ...
    # # #for same_person
    left_dir='./same_3persons/full/left/'
    right_dir='./same_3persons/full/right/'
    ...
    output_folder='./same_3persons_singleframes'
    ...
    result_left_dir='./same_3persons/full/left/'
    result_right_dir='./same_3persons/full/right/'
    ...
    ## 1-14 is person1  15-44 is person2  45-74 is person3 75-104 is person123 mixed
        ...
        if file_name.endswith('.jpg'):
            ...
            if os.path.exists(right_file_path):
                ...
                if result==(None, None, None, None, None):
                    # print('this frame is out of effective range')
                    recent_five_frames=[]
                    recent_five_frame_pointer=0
                    ...
                    # mean_frame, range_frame, part_frame, front, conf_sums_local=stereo.task(frameR, frameL, waitKey=0,show_depth=False, 
                    # use_augmentation=False, left_file_path=left_file_path, num_aug=num_aug, output_folder=output_folder, pick_biggest_box=True,run_onetime=True,effective_range=effective_range, max_min=max_min)


                    # # print(mean_frame, range_frame, part_frame)
                    # print('mean for this frame is: ', mean_frame)
                    # print('range for this frame is: ', range_frame)
                    # print('part for this frame is: ', part_frame)

                    

                    this_frame=Frame(mean_frame, range_frame, part_frame)

                    if len(recent_five_frames)==0:
                        recent_five_frames.append(this_frame)
                        recent_five_frame_pointer+=1
                    else:
                        means_a=recent_five_frames[recent_five_frame_pointer-1].mean_local
                        ranges_a=recent_five_frames[recent_five_frame_pointer-1].range_local
                        
                        means_b=this_frame.mean_local
                        ranges_b=this_frame.range_local

                        if(intersect(means_a, ranges_a, means_b, ranges_b, 0.5, map_probs)[0]>1.000001e-10):
                            # print("yes its matching!")
                            recent_five_frames.append(this_frame)
                            recent_five_frame_pointer+=1
                            if recent_five_frame_pointer==4:
                                mature=True

                    # print('frame number is: ', file_name)
                    # print('length of recent_five_frame is: ',len(recent_five_frames))
                    # print('local_database count is: ',len(local_database)) 
                    # print(file_name)      
                    if mature:
                        print("got matured one", "frame at: ", file_name, "count of local_database: ", len(local_database))
                        local_database=merge_recent_five(local_database, recent_five_frames)
                        recent_five_frames=[]
                        recent_five_frame_pointer=0
                        mature=False
                    # print(file_name)

                    # print('length of local_database is: ',len(local_database))
    ...
    for i in local_database:
        ...
        print('---')
                ...
                # #if this one is useful save it to a folder
                # if usable==1:

                #     #save top files, then pick top3
                #     good_image_name[base_filename]=conf_sums_local



                #     #use a dic for filename and conf_sums_local


                # else:
                #     print('keep finding new images')
    ...
    # sorted_items = sorted(good_image_name.items(), key=lambda x: x[1], reverse=True)[:top_ones]

    # top_keys = [item[0] for item in sorted_items]
    # top_values = [item[1] for item in sorted_items]

    # print("Top 3 keys:", top_keys)
    # print("Corresponding values:", top_values)

    # for i in top_keys:
    #     left_file_save=f"{i}.jpg"
    
    #     right_file_save=left_file_save.replace('left','right')

    #     copy_file(left_dir, result_left_dir, left_file_save)
    #     copy_file(right_dir, result_right_dir, right_file_save)
    ...
    # result_left_files = sorted(os.listdir(result_left_dir))
    # result_left_files = sorted(result_left_files, key=sort_numerically)

    # # print(result_left_files)

    # for file_name in result_left_files:
    #     # Make sure we're dealing with .jpg files
    #     if file_name.endswith('.jpg'):
    #         # Construct the corresponding right file name by replacing 'left' with 'right'
    #         right_file_name = file_name.replace('left', 'right')
            
    #         left_file_path = os.path.join(result_left_dir, file_name)
    #         right_file_path = os.path.join(result_right_dir , right_file_name)
    ...
    #             frame_vectors,fronts,conf_sum_persons=stereo.task(frameR, frameL, waitKey=0,show_depth=False, 
    #                 use_augmentation=False, left_file_path=left_file_path, num_aug=num_aug, output_folder=output_folder, pick_biggest_box=True,run_onetime=True,effective_range=effective_range, max_min=max_min)

    #             for i in frame_vectors:
    #                 for j in i:
    #                     j=float(j)

    #             clean_vector=frame_vectors
    #             for i in clean_vector:
    #                 for j in i:
    #                     if math.isnan(j) or math.isinf(j):
    #                         j=0

    #             vecs.append(clean_vector)
    #             print('this frame is: ', right_file_path, 'the framevector is: ', clean_vector, fronts, conf_sum_persons )
    #             print('-----------')
